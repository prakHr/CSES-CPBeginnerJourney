...
ll x,sum[SIZE],w[SIZE],wL[SIZE],wR[SIZE];

void pull(int i){
	w[i]=max(max(w[2*i],w[2*i+1]),wR[2*i]+wL[2*i+1]);
	wL[i]=max(wL[2*i],sum[2*i]+wL[2*i+1]);
	wR[i]=max(wR[2*i+1],sum[2*i+1]+wR[2*i]);
