...
vector<int> G[maxN];
// dist stores the distance of the node from root
...
int32_t main() {
	...
	// last now contains one of the leaf node
	// last = leaf node
	// run a dfs again from last to another leaf node
	// not to count last twice distance[last]=0
	dist[last]=0;
	dfs(last);
	for(int i=1;i<=N;i++){
		if(dist[i]>dist[last]){
