...
// idea :- only look into four directions 1.row 2.col 3.diagonally up 4.diagonally down
const int N=8;
char c;
int ans;
bool vis[N][N],input[N][N];
bool isValid(int x,int y){
	bool ok=true;
	REP(i,N){
		if(vis[x][i] or vis[i][y])ok=false;
	}
	for(int i=0;x-i>=0 and y-i>=0;i++){
		if(vis[x-i][y-i])ok=false;
	}
	for(int i=0;x-i>=0 and y+i<N;i++){
		if(vis[x-i][y+i])ok=false;
	}
	return ok;
}
bool check(){
	REP(i,N){
		REP(j,N){
			if(vis[i][j] and input[i][j])return false;
		}
	}
	return true;
}
