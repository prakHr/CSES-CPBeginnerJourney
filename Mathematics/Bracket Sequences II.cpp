...
// idea is from :- https://www.youtube.com/watch?v=uc0io6icKfY
...
unsigned long long modInverse(int n,int mod){
	return powerMod(n,mod-2,mod);
}
int binomialCoefficients(int a,int b,int MOD){
	vi fact(N,1);
	FOR(i,1,N,1){
		fact[i]=i*fact[i-1];
		fact[i]%=MOD;
	...
	return (((fact[a]*modInverse(fact[b],MOD))%MOD)*modInverse(fact[a-b],MOD))%MOD;
}
...
//       		cout<<"Here 1";
       ...
       REP(i,s.size()){
       		if(s[i]=='(')numOpeningBrackets++;
       		else numClosingBrackets++;
...
//	   		cout<<"Here 3";
	   ...
	   int m=n-s.size();
	   int remainingClosedB = (m+numOpeningBrackets-numClosingBrackets)/2;
	   int remainingOpeningB = m-remainingClosedB;
	   cout<<(binomialCoefficients(m,remainingClosedB,MOD)-binomialCoefficients(m,remainingClosedB+1,MOD)+MOD)%MOD<<endl;
	}
