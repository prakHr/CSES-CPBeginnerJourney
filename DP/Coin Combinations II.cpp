...
// either exclude or include c1,c2,...cn
// dp(i,x): valid ways to make x using c1,...ci coins
// final ans :-dp(N,x)
...
//	dp[0][0]=1;
	...
	for(int i=1;i<=n;i++){
		for(int j=0;j<=x;j++){
			// if j=sum of coins = 0 then there is one way to choose no coin at all
			if(j==0)dp[i][j]=1;
			// if(j-coins[i]<0 or i-1<0)break;
			else{
