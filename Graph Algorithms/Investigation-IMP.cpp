...
void djikstra(int src){
	...
	while(!pq.empty()){
		...
		visited[vertex]=true;	
		for(auto [cost,child]:edge[vertex]){
			int alternativeCost=Distance[vertex]+cost;
			if(alternativeCost==Distance[child]){
				num[child]=(num[child]+num[vertex])%MOD;
				minflights[child]=min(minflights[child],
				minflights[vertex]+1);
				maxflights[child]=max(maxflights[child],
				maxflights[vertex]+1);
			}
			else if(alternativeCost<Distance[child]){
				num[child]=num[vertex];
				minflights[child]=minflights[vertex]+1;
				maxflights[child]=maxflights[vertex]+1;
				pq.push({Distance[child]=alternativeCost,child});
			}
		}
	}
}
int32_t main(){
    ...
    for(int i=0;i<m;i++){
    	int start,end,cost;
    	cin>>start>>end>>cost;
    	edge[start].pb({cost,end});
	}
	memset(Distance+1,0x3f,n*sizeof(long long));
	djikstra(1);
	cout<<Distance[n]<<" "<<num[n]<<" "<<minflights[n]<<" "<<maxflights[n];
	return 0;
