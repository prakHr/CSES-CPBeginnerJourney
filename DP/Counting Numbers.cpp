...
// another variable to keep track of leading zeros
// dp(N,x,leading_zeros,tight)
// dp(N,-1,1,1)
ll dp[20][10][2][2];
int solve(string& number,int n,int x,int leading_zeros,bool tight){
	if(n==0)return 1;
	...
	for(int dig=lb;dig<=ub;dig++){
		if(dig==x and leading_zeros==0)
			continue;
		ans+=solve(number,n-1,dig,leading_zeros and dig==0,tight and(dig==ub));
	}
