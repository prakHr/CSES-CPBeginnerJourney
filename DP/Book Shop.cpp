...
// B1,B2,B3...Bn
// N books, X money
// either buy or not buy Nth book
// 0(N-1,X) 1(N-1,X-Cn) to maximize number of pages in 0 and 1 
// dp(i,x): max. no. of pages I can buy from ith book and xth money
// ans = dp(N,X)
// dp(i,x)=max(dp(i-1,x),pagesi+dp(i-1,x-costi))
// if x==0 then dp(i,x)=0 i cannot buy any book
// if i==0 then dp(i,x)=0 no books found to buy
void Solution() {
	int n,x;
	cin>>n>>x;
	int dp[n+1][x+1];
	...
	for(int i=0;i<n+1;i++){
		for(int j=0;j<x+1;j++){
			if(j==0 or i==0)dp[i][j]=0;
				...
				int op2 = (j-cost[i]<0) ? 0:pages[i]+dp[i-1][j-cost[i]];
				dp[i][j]=max(op1,op2);
			}
