...
// idea :- https://www.youtube.com/watch?v=8FdstVAkC5s
void Solution() {
	int n,k;
	cin>>n>>k;
	vi arr(n);
	...
	REP(i,n){
		while(j<n and ((int)occurences.size()<k or occurences.count(arr[j])>0)){
			occurences[arr[j]]++;
			j++;
		}
		ans+=j-i;
		occurences[arr[i]]--;
		if(occurences[arr[i]]==0)occurences.erase(arr[i]);
	}
	cout<<ans<<endl;
}	
