...
/*------------------------------------------------*/
 ...
 // idea :- we have to iterate from backward
 // use bit manipulation to update an array 
 // variable is total_Sum of an array
 // link :- https://www.youtube.com/watch?v=PXdOyrbNr78
...
void Solution() {
	...
	REP(i,n){
		for(int value=sum;value>=values[i];value--){
			reachable[value] = reachable[value] or reachable[value-values[i]];
		}
	}
	vi v;
	
	REP(i,sum+1){
		if(i!=0 and reachable[i]){
			v.pb(i);
		}
	}
	cout << v.size()<<endl;
