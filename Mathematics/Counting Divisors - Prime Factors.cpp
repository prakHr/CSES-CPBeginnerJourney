...
void Solution() {
	int n;
	cin>>n;
	vector<pair<int,int>> powerOfPrimeFactors;
	for(int i=2;i*i<=n;i++){
		if(n%i==0){
			int power=0;
			while(n%i==0){
			...
			powerOfPrimeFactors.push_back({i,power});
		}
	}
	if(n!=1)powerOfPrimeFactors.push_back({n,1});
	cout<<"Prime Factorisation of n = "<<n<<endl;
	for(int i=0;i<powerOfPrimeFactors.size();i++){
		int prime = powerOfPrimeFactors[i].ff;
		int exponent = powerOfPrimeFactors[i].ss;
