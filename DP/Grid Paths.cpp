...
// definition,final ans,recurrence relation,base case
// 1.dp(i,j) as number of valid paths from the cell(i,j) to (N,N)
// 2.ans = dp(1,1)
// 3.dp(i,j) = k1 ways + k2 ways = dp(i+1,j)+dp(i,j+1)
// 4.dp(N,N) = 1 base case 
void Solution() {
	int n;
	cin>>n;
	int grid[n+1][n+1];
