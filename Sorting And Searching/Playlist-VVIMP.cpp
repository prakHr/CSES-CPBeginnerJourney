...
// https://www.youtube.com/watch?v=D70lafXKnBU
// for n=1000 ,bruteforce solution
// for(int i=0;i<n;i++){
//			for(int j=i;j<n and !st.count(songsId[j]);j++){
//				st.insert(songsId[j]);
//				ans=max(ans,j-i+1);
//			}
//		}
...
void Solution() {
	...
	if(n==1){
		...
		int i=0,j=0,successiveSongs=0;
		while(i<n and j<n){
			while(j<n and !st.count(songsId[j])){
				st.insert(songsId[j]);
