...
// idea:-https://www.youtube.com/watch?v=__21DNroCLo
ll powerMod(int base ,int exponent,int mod){
	if(exponent==0)return 1;
	ll result = powerMod(base,exponent/2,mod);
	if(exponent%2==1){
		return (((result*result)%mod)*base)%mod;
	}
	else return (result*result)%mod;
}
int geometricSum(int base,int power,int MOD){
	ll numerator=(powerMod( base ,power+1,MOD)-1+MOD)%MOD;
	ll denominatorInverse=powerMod(base-1,MOD-2,MOD);
	return (numerator*denominatorInverse)%MOD;
}
void Solution() {
	int n;
	cin>>n;
	vi prime(n),exponent(n);
	REP(i,n){
		cin>>prime[i]>>exponent[i];
	}
	ll numberOfDivisors=1,sumOfDivisors=1;
	REP(i,n){
		numberOfDivisors=(numberOfDivisors*(exponent[i]+1))%MOD;
	}
	REP(i,n){
		sumOfDivisors=(sumOfDivisors*geometricSum(prime[i],exponent[i],MOD))%MOD;
	}
	...
	if(oddExponent){
		ll outerExponent = 1;
		REP(i,n){
			if(i==positionOfOddExponent)outerExponent = (outerExponent *(exponent[i]+1)/2)%(MOD-1);
			else outerExponent = (outerExponent *(exponent[i]+1))%(MOD-1);
		}
		REP(i,n){
			productOfDivisors = (productOfDivisors*powerMod(prime[i],(exponent[i]*outerExponent)%(MOD-1),MOD))%MOD;
		}
	}
	else{
		ll outerExponent = 1;
		...
		REP(i,n){
			productOfDivisors = (productOfDivisors*powerMod(prime[i],((exponent[i]/2)*outerExponent)%(MOD-1),MOD))%MOD;
		}
	}
