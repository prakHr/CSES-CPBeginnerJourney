...
ll x,sum,cnt=0;
map<ll,int> freq;
// idea:- divide into 2 all kinds of possible subsets
// subset A with sum in frequency table + subset B with sum in frequency table = target 
// we can take count from this frequency table as ans
// with hash value = target-subset B sum
...
void Solution() {
	...
	sort(t,t+N);
	if(N==1){
		if(x==t[0])cout<<1<<endl;
		else cout<<0<<endl;
	}
	else{
		for(int mask=0;mask<(1<<(N/2-1));mask++){
			sum=0;
			for(int pos=0;pos<N/2-1;pos++){
				if(mask & (1<<pos))sum+=t[pos];
			}
			freq[sum]+=1;
		}
		for(int mask=0;mask<(1<<(N-(N/2-1)));mask++){
			sum=0;
			for(int pos=0;pos<(N-(N/2-1));pos++){
				if(mask & (1<<pos))sum+=t[N/2-1+pos];
			}
